...
const WHATSAPP_NUMBER = '919008822802'

function buildWhatsAppMessage(cartItems, totalPrice) {
      ...
      >
        ...
        {cartItems.length > 0 && (
          <div className="border-t border-earth-100 px-4 py-4 bg-white">
            ...
            <a
              href={`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(buildWhatsAppMessage(cartItems, totalPrice))}`}
              target="_blank"
              rel="noopener noreferrer"
              className="w-full min-h-[48px] bg-[#25D366] hover:bg-[#1ebe5d] active:bg-[#17a84f] text-white font-bold rounded-xl text-sm transition-colors flex items-center justify-center gap-2"
            >
